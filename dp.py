    ...
    def analysis(self):
        ...
        for i in range(len(expression)):
            if (expression[i] >= 'a' and expression[i] <= 'z') or expression[i] in ops:
                continue
            if expression[i] == '|':
                if i != 0 and (i != len(expression) - 1 and expression[i - 1] <= 'z' and expression[i - 1] >= 'a' and
                               expression[i + 1] <= 'z' and expression[i + 1] >= 'a') or expression[i - 1] == ')':
                    continue
                if expression[i - 1] == '*':
                    if i - 1 != 0 and (
                            (expression[i - 2] <= 'z' and expression[i - 2] >= 'a') or expression[i - 2] == ')'):
                        continue
                    else:
                        legel = False
                        break
...
class UsingTest_(QMainWindow, Ui_MainWindow):
    def __init__(self, *args, **kwargs):
        super(UsingTest_, self).__init__(*args, **kwargs)
